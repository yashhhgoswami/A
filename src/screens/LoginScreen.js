...
const LoginScreen = () => {
  ...
  useEffect(() => {
    ...
    const role = roleFromState || roleFromStorage || 'alumni';
    setSelectedRole(role);
  }, [location]);

  const getRoleDisplayInfo = (role) => {
    const roleInfo = {
      alumni: { title: 'Alumni', color: 'purple', available: true },
      student: { title: 'Student', color: 'blue', available: true },
      institute: { title: 'Institute', color: 'green', available: true },
      recruiter: { title: 'Recruiter', color: 'orange', available: true }
    };
    return roleInfo[role] || roleInfo.alumni;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!roleInfo.available) {
      setError(`${roleInfo.title} portal is coming soon! Please try the Alumni portal for now.`);
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-purple-600 via-purple-700 to-indigo-800 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-center px-12 text-white">
          <div className="mb-8">
            <h1 className="text-5xl font-bold mb-4">Welcome to AlumniConnect</h1>
            <p className="text-xl opacity-90 leading-relaxed">
              {selectedRole === 'alumni' && "Connect with your alumni network and manage your professional journey"}
              {selectedRole === 'student' && "Access mentorship and connect with alumni for career guidance"}
              {selectedRole === 'institute' && "Manage alumni relations and institutional connections"}
          ...
          <div className="space-y-4">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-lg">Advanced Alumni Management</span>
            </div>
            <div className="flex items-center space-x-3">
              <div className="w-6 h-6 bg-white bg-opacity-20 rounded-full flex items-center justify-center">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
